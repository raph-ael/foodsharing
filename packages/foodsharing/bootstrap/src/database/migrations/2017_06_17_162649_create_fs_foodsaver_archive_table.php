...
class CreateFsFoodsaverArchiveTable extends Migration {
	...
	public function up()
	{
		Schema::create('fs_foodsaver_archive', function(Blueprint $table)
		{
			$table->increments('id');
			$table->integer('bezirk_id')->unsigned()->index('foodsaver_FKIndex2');
			$table->string('position')->default('');
			$table->boolean('verified')->default(0);
			$table->dateTime('last_pass');
			$table->string('new_bezirk', 120);
			...
			$table->text('about_me_public', 65535);
			...
			$table->integer('stat_buddycount')->unsigned();
			...
			$table->date('sleep_from');
			$table->date('sleep_until');
			$table->text('sleep_msg', 65535);
			$table->date('last_mid');
	...
	{
		Schema::drop('fs_foodsaver_archive');
	}
