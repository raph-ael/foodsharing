...
use Foodsharing\TranslationManager\Http\Controllers\LocaleController;
...
use Foodsharing\TranslationManager\Models\UserLocales;
...
class Controller extends BaseController
{
    ...
    {
        $this->package = \Foodsharing\TranslationManager\ManagerServiceProvider::PACKAGE;
    ...
    protected function keyOp($group, $op = 'preview')
    {
        ...
        if ($userLocales && !in_array($group, $this->manager->config(Manager::EXCLUDE_GROUPS_KEY)) && $this->manager->config('admin_enabled')) {
            ...
            if (!$group) {
                $errors[] = trans('translation-manager.keyop-need-group');
            } elseif (count($srckeys) !== count($dstkeys) && ($op === 'copy' || $op === 'move' || count($dstkeys))) {
                $errors[] = trans('translation-manager.keyop-count-mustmatch');
            } elseif (!count($srckeys)) {
                $errors[] = trans('translation-manager.keyop-need-keys');
            } else {
                if (!count($dstkeys)) {
                ...
                foreach ($keys as $src => $dst) {
                    ...
                    if ($dst !== null) {
                        ...
                        if ((substr($src, 0, 1) === '*') !== (substr($dst, 0, 1) === '*')) {
                            $keyerrors[] = trans('translation-manager.keyop-wildcard-mustmatch');
                        }
                        ...
                        if ((substr($src, 0, 1) === '*') && (substr($src, -1, 1) === '*')) {
                            $keyerrors[] = trans('translation-manager.keyop-wildcard-once');
                        }
                    }
                            ...
                            foreach ($rows as $row) {
                                if ($this->isLocaleEnabled($row->locale)) {
                                    ...
                                    if (!empty($to_delete)) {
                                        ...
                                        if ($to_delete) {
                                            //$this->getConnection()->update("UPDATE translations SET is_deleted = 1 WHERE id IN ($to_delete)");
            ...
            $errors[] = trans('translation-manager.keyops-not-authorized');
        }
