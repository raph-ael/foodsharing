    ...
    public function inPlaceEditLink($t, $withDiff = false, $key = null, $locale = null, $useDB = null, $group = null)
    {
        try {
            ...
            if ($t) {
                ...
                $title = parent::get('translation-manager.enter-translation');

                if ($t->value === null) $t->value = ''; //$t->value = parent::get($key, $replace, $locale);

                $action = \URL::action(ManagerServiceProvider::CONTROLLER_PREFIX . 'Foodsharing\TranslationManager\Controller@postEdit', array($t->group));

                $result = '<a href="#edit" class="fstrans_editable status-' . ($t->status ?: 0)
                    . ' locale-' . $t->locale
