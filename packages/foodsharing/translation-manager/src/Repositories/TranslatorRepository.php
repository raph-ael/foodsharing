...
use Foodsharing\TranslationManager\Repositories\Interfaces\ITranslatorRepository;
...
abstract class TranslatorRepository implements ITranslatorRepository
{
    ...
    public function __construct(Translation $translation)
    {
        ...
        $this->tableRenameNeeded = $this->tableName != 'translations';
    }
     ...
     *
     * @param string $sql SQL query where to replace every occurrence of " translations " with the actual table name
     *
     * @return string of the modified query
     */
    protected function adjustTranslationTable($sql)
    {
        $adjustedSql = $this->tableRenameNeeded ? str_replace(' translations ', ' ' . $this->tableName . ' ', $sql) : $sql;
        return $adjustedSql;
    }

    public function updateIsDeletedByIds($rowIds)
    {
        $this->connection->update($this->adjustTranslationTable("UPDATE translations SET is_deleted = 1 WHERE is_deleted = 0 AND id IN ($rowIds)"));
    }

    public function setNotUsedForAllTranslations()
    {
        $this->connection->affectingStatement($this->adjustTranslationTable(<<<SQL
            UPDATE translations SET was_used = 0 WHERE was_used <> 0
SQL
        ));
    }
    ...
    {
        $this->connection->affectingStatement($this->adjustTranslationTable(<<<SQL
UPDATE translations SET status = ?, is_deleted = 0, updated_at = ? WHERE id IN ($translationIds)
SQL
        ), [$status, $updated_at]);
    }
    ...
    public function deleteTranslationsForIds($translationIds)
    {
        $this->connection->unprepared($this->adjustTranslationTable(<<<SQL
          DELETE FROM translations WHERE id IN ($translationIds)
SQL
        ));
    }
    ...
    public function updateValuesByStatus()
    {
        ...
        $this->connection->affectingStatement($this->adjustTranslationTable(<<<SQL
UPDATE translations SET saved_value = value, status = ? WHERE (saved_value <> value || status <> ?)
SQL
