...
namespace Foodsharing\TranslationManager\Repositories;

use Foodsharing\TranslationManager\Models\Translation;
    ...
    public function updateUsedTranslationsForGroup($keys, $group)
    {
        ...
        if ($setKeys) {
            $this->connection->affectingStatement($this->adjustTranslationTable(<<<SQL
                UPDATE translations SET was_used = 1 WHERE was_used <> 0 AND (`group` = ? OR `group` LIKE ? OR `group` LIKE ?)
SQL
        ...
        if ($resetKeys) {
            $this->connection->affectingStatement($this->adjustTranslationTable(<<<SQL
            UPDATE translations SET was_used = 0 WHERE was_used <> 0 AND (`group` = ? OR `group` LIKE ? OR `group` LIKE ?) AND `key` IN ($resetKeys)
SQL
            ), [$group, 'vnd:%.' . $group, 'wbn:%.' . $group]);
        }
    }
...
UPDATE translations SET is_deleted = ? WHERE is_deleted = $whereClause AND `group` = ? AND `key` = ?
SQL
        ), [$value, $group, $key]);
    }
    ...
    {
        $this->connection->update($this->adjustTranslationTable("UPDATE translations SET `group` = ?, `key` = ?, status = 1 WHERE id = ?"), [$group, $key, $id]);
    }
...
SELECT * FROM translations WHERE locale = ? AND `group` = ?
SQL
...
SELECT source FROM translations WHERE `group` = ? AND `key` = ?
SQL
        ), [$group, $key]);
    }
    ...
    {
        // $values is an
        $sql = $this->adjustTranslationTable("INSERT INTO translations (status, locale, `group`, `key`, value, created_at, updated_at, source, saved_value, is_deleted, was_used) VALUES " . implode(",", $values));
    ...
    {
        if (!$group) {
            $this->connection->affectingStatement($this->adjustTranslationTable("DELETE FROM translations WHERE is_deleted = 1"));
        } else {
            $this->connection->affectingStatement($this->adjustTranslationTable("DELETE FROM translations WHERE is_deleted = 1 AND `group` = ?"), [$group]);
        }
    }

    public function deleteTranslationByGroup($group)
    {
        $this->connection->affectingStatement($this->adjustTranslationTable("DELETE FROM translations WHERE `group` = ?"), [$group]);
    }

    public function updateValueInGroup($group)
    {
        $this->connection->affectingStatement($this->adjustTranslationTable(<<<SQL
UPDATE translations SET saved_value = value, status = ? WHERE (saved_value <> value || status <> ?) AND `group` = ?
SQL
...
SELECT  
    ...
    FROM translations rt WHERE (`key` LIKE ? OR value LIKE ?) $displayWhere
UNION ALL
...
FROM (SELECT DISTINCT locale FROM translations  WHERE 1=1 $displayWhere) lt
    CROSS JOIN (SELECT DISTINCT `key`, `group` FROM translations  WHERE 1=1 $displayWhere) kt
WHERE NOT exists(SELECT * FROM translations  tr WHERE tr.`key` = kt.`key` AND tr.`group` = kt.`group` AND tr.locale = lt.locale)
      AND `key` LIKE ?
...
SELECT  
    ...
    ltm.is_auto_added
FROM translations ltm WHERE `group` = ? $displayWhere
UNION ALL
SELECT DISTINCT
    NULL id,
    NULL status,
    ...
    NULL is_auto_added
FROM
(SELECT * FROM (SELECT DISTINCT locale FROM translations WHERE 1=1 $displayWhere) lcs
    CROSS JOIN (SELECT DISTINCT `group`, `key` FROM translations WHERE `group` = ? $displayWhere) grp) m
WHERE NOT EXISTS(SELECT * FROM translations t WHERE t.locale = m.locale AND t.`group` = m.`group` AND t.`key` = m.`key`)
ORDER BY `key` ASC
SQL
...
FROM
    (SELECT sum(total) total, sum(changed) changed, sum(cached) cached, sum(deleted) deleted, `group`, locale
     FROM
         ...
         `group`, locale
                FROM translations lt WHERE 1=1 $displayWhere GROUP BY `group`, locale
          UNION ALL
          SELECT DISTINCT 0, 0, 0, 0, `group`, locale FROM (SELECT DISTINCT locale FROM translations WHERE 1=1 $displayWhere) lc
              CROSS JOIN (SELECT DISTINCT `group` FROM translations) lg) a
     GROUP BY `group`, locale) lcs
    JOIN (SELECT count(DISTINCT `key`) total_keys, `group` FROM translations WHERE 1=1 $displayWhere GROUP BY `group`) mx
...
FROM (SELECT * FROM translations WHERE 1=1 $displayWhere) lt
    JOIN
    (SELECT DISTINCT mt.`key`, BINARY mt.ru ru, BINARY mt.en en
     FROM (SELECT lt.`group`, lt.`key`, group_concat(CASE lt.locale WHEN '$primaryLocale' THEN VALUE ELSE NULL END) en, group_concat(CASE lt.locale WHEN '$translatingLocale' THEN VALUE ELSE NULL END) ru
           FROM (SELECT value, `group`, `key`, locale FROM translations WHERE 1=1 $displayWhere
                 UNION ALL
                 SELECT NULL, `group`, `key`, locale FROM ((SELECT DISTINCT locale FROM translations WHERE 1=1 $displayWhere) lc
                     CROSS JOIN (SELECT DISTINCT `group`, `key` FROM translations WHERE 1=1 $displayWhere) lg)
                ) lt
...
SQL
    ...
    {
        return $this->connection->select($this->adjustTranslationTable(<<<SQL
SELECT GROUP_CONCAT(id SEPARATOR ',') ids FROM translations tr
...
SQL
    ...
    {
        $translations = $this->getTranslationsTableName();

        if ((substr($src, 0, 1) === '*')) {
            if ($dst === null) {
                $rows = $this->connection->select($this->adjustTranslationTable($sql = <<<SQL
SELECT DISTINCT `group`, `key`, locale, id, NULL dst, NULL dstgrp FROM $translations t1
...
SQL
                ...
                $rows = $this->connection->select($this->adjustTranslationTable($sql = <<<SQL
SELECT DISTINCT `group`, `key`, locale, id, CONCAT(SUBSTR(`key`, 1, CHAR_LENGTH(`key`)-?), ?) dst, ? dstgrp FROM $translations t1
WHERE `group` = ? AND `key` LIKE BINARY ? AND locale IN ($userLocales)
AND NOT exists(SELECT * FROM $translations t2 WHERE t2.value IS NOT NULL AND t2.`group` = ? AND t1.locale = t2.locale
...
SQL
                ...
                $rows = $this->connection->select($this->adjustTranslationTable($sql = <<<SQL
SELECT DISTINCT `group`, `key`, locale, id, ? dst, ? dstgrp FROM $translations t1
WHERE `group` = ? AND `key` LIKE BINARY ? AND locale IN ($userLocales)
AND NOT exists(SELECT * FROM $translations t2 WHERE t2.value IS NOT NULL AND t2.`group` = ? AND t1.locale = t2.locale AND t2.`key` LIKE BINARY ?)
ORDER BY locale, `key`
...
INSERT INTO $translations (status, locale, `group`, `key`, value, created_at, updated_at, source, saved_value, is_deleted, was_used) 
SELECT
    1 status,
    locale,
    ? `group`,
