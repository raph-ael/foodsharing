...
SQL
    ...
    {
        return $this->connection->select($this->adjustTranslationTable(<<<SQL
SELECT id FROM translations tr
...
WHERE "group" = ? AND "key" LIKE  ? AND locale IN ($userLocales)
AND NOT exists(SELECT * FROM $translations t2 WHERE t2.value IS NOT NULL AND t2."group" = ? AND t1.locale = t2.locale AND t2."key" LIKE ?)
ORDER BY locale, "key"
