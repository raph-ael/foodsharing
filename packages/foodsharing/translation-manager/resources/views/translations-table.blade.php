<table id="translations" class="table table-condensed table-striped table-translations">
    <thead>
        <tr>
            ...
            ?>
            <th width="15%">@lang('translation-manager.key')
            ...
            <th width="<?=$mainWidth?>%"><?= $locale ?>&nbsp;
                <?= ifEditTrans('translation-manager.auto-translate-disabled') ?>
                <?= ifEditTrans('translation-manager.auto-translate') ?>
                <a class="btn btn-xs btn-primary auto-translate" role="button" data-trans="<?=$col?>" data-locale="<?=$locale?>" <?= $isLocaleEnabled ? '' : 'disabled' ?>
                data-disable-with="<?=noEditTrans('translation-manager.auto-translate-disabled')?>"
                        href="#"><?= noEditTrans('translation-manager.auto-translate') ?></a>
                <?= ifEditTrans('translation-manager.auto-prop-case-disabled') ?>
                <a class="btn btn-xs btn-primary auto-prop-case" role="button" data-trans="<?=$col?>" data-locale="<?=$locale?>" <?= $isLocaleEnabled ? '' : 'disabled' ?>
                data-disable-with="<?=noEditTrans('translation-manager.auto-prop-case-disabled')?>"
                        href="#">Ab Ab <i class="glyphicon glyphicon-share-alt"></i> Ab ab
                </a>
                <!-- split button -->
                <!--
                <div class="btn-group">
                    <button type="button" class="btn btn-xs btn-primary">< ? = noEditTrans('translation-manager.auto-translate') ? ></button>
                    ...
                    <ul class="dropdown-menu">
                        ...
                        <li><a href="#">< ? = noEditTrans('translation-manager.auto-translate') ? ></a></li>
                    </ul>
                </div>
                -->
            </th>
...
?><a style="float: right;" href="<?= action($controller . '@postShowSource', [$group, encodeKey($key)]) ?>"
                        class="show-source-refs" data-method="POST" data-remote="true" title="@lang('translation-manager.show-source-refs')">
